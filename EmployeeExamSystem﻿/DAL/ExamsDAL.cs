...
namespace EmployeeExamSystem_.DAL
{
    internal class ExamsDAL
    {
        ...
        public DataTable GetAllExams()
        {
            ...
            {
                con.Open();
                string query = @"SELECT ExamID, ExamName, PeriodTimeInMinutes, EndDate, CreatedAt, LastModifiedDate, IsActive
                                 FROM Exams";
        ...
        public bool AddExam(string name, int periodTime, DateTime endDate, bool isActive)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                string query = @"INSERT INTO Exams 
                                 (ExamName, PeriodTimeInMinutes, EndDate, CreatedAt, LastModifiedDate, IsActive)
                                 VALUES (@Name, @Time, @EndDate, GETDATE(), NULL, @IsActive)";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
        ...
        public bool UpdateExam(int examId, string name, int periodTime, DateTime endDate, bool isActive)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                ...
                string query = @"UPDATE Exams
                                 SET ExamName=@Name, PeriodTimeInMinutes=@Time, EndDate=@EndDate, IsActive=@IsActive, LastModifiedDate=GETDATE()
                                 WHERE ExamID=@ID";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
